...
export class DocModalComponent implements OnInit {
  ...
  submitDoc() {
    ...
    if (this.page == 'hs') {
      localStorage.setItem('hsdoc', this.doc)
      this._location.back()
    }

    if (this.page == 'undergraduate') {
      localStorage.setItem('doc', this.doc)
      this._location.back()
    }

    if (this.page == 'applycourse') {
      const courseData = new FormData();
      let id: any = localStorage.getItem('applyId')
      courseData.append('id', id);
      courseData.append('apply_course_file_signed', this.doc);
      this._api.applyCourseDocument(courseData).subscribe(res => {
        if (res.status == '1') {
          this._loader.stopLoader('loader');
          this.Toast.fire({
            icon: 'success',
    ...
    if (this.page == 'amountFile') {
      const amountData = new FormData();
      let id: any = localStorage.getItem('applyId')
      amountData.append('id', id);
      amountData.append('deposit_amount_file', this.doc);
      this._api.applyDepositAmountDocument(amountData).subscribe(res => {
        if (res.status == '1') {
          this._loader.stopLoader('loader');
          this.Toast.fire({
            icon: 'success',
    ...
    if (this.page == 'applyVisa') {
      const amountData = new FormData();
      let id: any = localStorage.getItem('applyId')
      amountData.append('id', id);
      amountData.append('apply_visa_file', this.doc);
      this._api.applyVisaDocument(amountData).subscribe(res => {
        if (res.status == '1') {
          this._loader.stopLoader('loader');
          this.Toast.fire({
            icon: 'success',
